...
- **Scheduler**: `Bun.cron` every minute, in-process (native Bun API, no dependency). `startScheduler()` in `src/lib/server/scheduler.ts` is idempotent — one shared `globalThis.__aghara_scheduler_runtime` (`running` + `lastTickAt`) is both the double-start guard and what the health check reads — and is called from `src/hooks.server.ts` when not building. No-overlap is built in; after a sleep the next tick's `publishDue()` catches up all due rows.
...
- `src/lib/server/services/`: `posts.ts` (`createPost`, `listScheduled`, `publishNow`, `cancelScheduled`), `accounts.ts`, `tokens.ts` (hash + show-once), `billing.ts` (`getPlan`, `checkLimit`), `publisher.ts` (`publishDue`, claim-then-publish, 3 retries), `health.ts` (`getHealthSnapshot` — powers public `/api/v1/health` + the `/health` screen), `crypto.ts`, `errors.ts`.
